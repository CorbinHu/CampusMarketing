...
(function ($) {
    $.extend({
    	...
    	modal: {
            ...
            // 弹出层指定宽度
            open: function (title, url, width, height, btn, shadeClose, closeTag, callback) {
            	//如果是移动端，就使用自适应大小弹窗
        ...
        operate: {
			...
			accept: function(id) {
				table.set();
				$.modal.confirm("确定接受" + table.options.modalName + "活动吗？", function() {
					var url = $.common.isEmpty(id) ? table.options.acceptUrl : table.options.acceptUrl.replace("{id}", id);
					if(table.options.type == table_type.bootstrapTreeTable) {
						$.operate.get(url);
					} else {
						var data = { "ids": id };
